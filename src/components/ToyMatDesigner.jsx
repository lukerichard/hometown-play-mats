...
const ToyMatDesigner = () => {
  ...
  const colorSchemes = {
    classic: { color: '#3DAEF5', name: 'Sky Cruiser' },
    muted: { color: '#87CEEB', name: 'Clear Sky' },
    neon: { color: '#A76BDB', name: 'Wildcard Purple' }
  };
  ...
  const fetchSuggestions = async (query) => {
    ...
    try {
      ...
      if (data.features && data.features.length > 0) { setSuggestions(data.features); setShowSuggestions(true); }
      else { setSuggestions([]); setShowSuggestions(false); }
    } catch (error) { console.error('Autocomplete error:', error); setSuggestions([]); setShowSuggestions(false); }
  };
  ...
  const rotateLeft = () => {
    setRotation((prev) => { const n = (prev - 45) % 360; return n < 0 ? n + 360 : n; });
  };
  const rotateRight = () => { setRotation((prev) => (prev + 45) % 360); };

  const getSelectionBoxDimensions = () => {
    const DPI = 96;
    const SCALE = 2;
    const dimensions = { small: { widthInches: 2, heightInches: 1 }, medium: { widthInches: 2, heightInches: 1.5 }, large: { widthInches: 3, heightInches: 2 } };
    const sizeKey = matSizes[matSize].name.toLowerCase();
    const size = dimensions[sizeKey] || dimensions.small;
    return { width: size.widthInches * DPI * SCALE, height: size.heightInches * DPI * SCALE };
  };

  const handleGenerateMat = () => {
    if (!mapInstance) { alert('Map is still loading. Please wait a moment and try again.'); return; }
    try {
      const canvas = mapInstance.getCanvas();
      if (!canvas) { alert('Error capturing map. Please try again.'); return; }
      ...
      setShowPreview(true);
    } catch (error) { console.error('Error capturing map:', error); alert('Error capturing map. Please try again.'); }
  };
  ...
  const handleSaveMat = async () => {
    if (!currentUser) { const shouldSignup = confirm('You need an account to save mats. Create an account now?'); if (shouldSignup) navigate('/signup'); return; }
    if (!matName.trim()) { alert('Please enter a name for your mat'); return; }
    if (!previewImage) { alert('Please generate a preview first'); return; }
    setSaving(true);
    try {
      const matData = { name: matName.trim(), matSize, colorScheme, rotation, mapCenter, mapZoom, address, previewImageUrl: previewImage };
      if (savedMatId) { await updateMat(savedMatId, matData); alert('Mat updated successfully!'); }
      else { const newMatId = await saveMat(currentUser.uid, matData); setSavedMatId(newMatId); alert('Mat saved successfully!'); }
      setShowSaveDialog(false);
      setMatName('');
    } catch (error) { console.error('Error saving mat:', error); alert('Failed to save mat. Please try again.'); }
  ...
  const handleAddToCart = async () => {
    if (!currentUser) { const shouldSignup = confirm('You need an account to add items to cart. Create an account now?'); if (shouldSignup) navigate('/signup'); return; }
    if (!previewImage) { alert('Please generate a preview first'); return; }
    try {
      let matIdToAdd = savedMatId;
      if (!matIdToAdd) {
        const defaultName = `Mat - ${address.substring(0, 30)}${address.length > 30 ? '...' : ''}`;
        const matData = { name: defaultName, matSize, colorScheme, rotation, mapCenter, mapZoom, address, previewImageUrl: previewImage };
  ...
  const handleUpdateQuantity = async (newQuantity) => {
    ...
    try {
      if (newQuantity <= 0) { await removeFromCart(currentCartItem.id); }
      else { await updateCartQuantity(currentCartItem.id, newQuantity); }
    } catch (error) { console.error('Error updating quantity:', error); alert('Failed to update quantity. Please try again.'); }
  };

  return (
    <>
      <div className="h-screen overflow-hidden" style={{ background: '#FDF8F0' }}>
          ...
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 rounded-xl flex items-center justify-center shrink-0"
              ...
              <svg width="20" height="20" viewBox="0 0 20 20" fill="none">
                <rect x="4" y="2" width="12" height="16" rx="2" fill="#6B6B6B" />
                <line x1="10" y1="4" x2="10" y2="16" stroke="#FFCC00" strokeWidth="1.5" strokeDasharray="2 2" />
                <circle cx="7" cy="8" r="1.5" fill="#FF8C42" />
                <circle cx="13" cy="13" r="1.5" fill="#3DAEF5" />
              </svg>
            </div>
            <div>
              ...
              <div className="flex gap-3">
                <div className="flex-1 relative">
                  <div className="absolute left-4 top-1/2 -translate-y-1/2 z-10 pointer-events-none">
                    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#B0A999" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                  ...
                  <input
                    ...
                    className="w-full py-3 pl-11 pr-5 bg-white border-2 border-border rounded-xl text-sm font-medium text-text outline-none transition-all duration-200 focus:border-primary"
                    ...
                    <div className="absolute top-full left-0 right-0 mt-2 bg-white border-2 border-border rounded-xl overflow-hidden z-[99999] max-h-[280px] overflow-y-auto animate-fade-in"
                      ...
                      {suggestions.map((suggestion, index) => (
                        <div
                          ...
                          className="px-4 py-3 cursor-pointer border-b border-border/50 last:border-b-0 transition-colors duration-150"
                          style={{ }}
                          onMouseEnter={(e) => e.currentTarget.style.background = '#D6EFFF40'}
                          onMouseLeave={(e) => e.currentTarget.style.background = 'transparent'}
                        >
                          <div className="text-sm font-semibold text-text mb-0.5">{suggestion.text}</div>
                          <div className="text-xs text-text-light">{suggestion.place_name}</div>
                        </div>
                      ))}
                    </div>
                  )}
                ...
                <button
                  ...
                  className="px-6 py-3 text-white border-none rounded-full text-sm font-bold cursor-pointer transition-all duration-200 hover:-translate-y-0.5 disabled:cursor-not-allowed disabled:opacity-60"
                  style={{
                    fontFamily: "'DM Sans', sans-serif",
                    background: isSearching ? '#B0A999' : '#3DAEF5',
                    boxShadow: isSearching ? 'none' : '0 2px 8px rgba(61, 174, 245, 0.3)',
                  }}
                >
      ...
      {showPreview && (
        <MatPreview
          ...
          onSave={() => {
            if (!matName) {
              if (address) { setMatName(address); }
              else { setMatName(`My Mat - ${new Date().toLocaleDateString()}`); }
            }
          ...
          <div className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-white rounded-2xl p-8 max-w-md w-[90%] z-[10000] border-2 border-border animate-fade-in"
            ...
            <div className="mb-5">
              ...
              <input
                ...
                autoFocus
                className="w-full py-3 px-4 border-2 border-border rounded-xl text-sm font-medium outline-none transition-all duration-200 focus:border-primary"
                style={{ fontFamily: "'DM Sans', sans-serif", boxSizing: 'border-box' }}
              />
            </div>
            ...
            <div className="flex gap-3">
              <button
                onClick={() => { setShowSaveDialog(false); setMatName(''); }}
                disabled={saving}
                className="flex-1 py-3 bg-white border-2 border-border rounded-full text-sm font-bold text-text-light cursor-pointer transition-all duration-200 hover:border-primary disabled:cursor-not-allowed"
              ...
              <button
                ...
                disabled={saving}
                className="flex-1 py-3 text-white border-none rounded-full text-sm font-bold cursor-pointer transition-all duration-200 hover:-translate-y-0.5 disabled:cursor-not-allowed disabled:opacity-60"
                style={{
                  fontFamily: "'DM Sans', sans-serif",
                  background: saving ? '#B0A999' : '#3DAEF5',
                  boxShadow: saving ? 'none' : '0 2px 8px rgba(61, 174, 245, 0.3)',
                }}
              >
